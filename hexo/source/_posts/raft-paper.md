---
title: 寻找一种易于理解的一致性算法
date: 2022-02-25 13:06:13
tags: [分布式系统, 分布式存储]
categories: 分布式系统
---
...
Raft 是一种为了管理复制日志的一致性算法。它提供了和 Paxos 算法相同的功能和性能，但是它的算法结构和 Paxos 不同，使得 Raft 算法更加容易理解并且更容易构建实际的系统。为了提升可理解性，Raft 将一致性算法分解成了几个关键模块，例如领导人选举、日志复制和安全性。同时它通过实施一个更强的一致性来减少需要考虑的状态的数量。一项用户研究的结果表明，对于学生而言，Raft 算法比 Paxos 算法更加容易学习。Raft 算法还包括一个新的机制来允许集群成员的动态改变，它利用重叠的大多数来保证安全性。<!--more-->
...
![图 1 ](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-1.png)
...
![图 2](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-2.png)
...
| 日志匹配原则(Log Matching)           | 如果两个日志在某一相同索引位置日志条目的任期号相同，那么我们就认为这两个日志从头到该索引位置之间的内容完全一致（5.3 节） |
...
![图 3 ](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-3.png)
...
![图 4 ](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-4.png)

> 图 4：服务器状态。跟随者只响应来自其他服务器的请求。如果跟随者接收不到消息，那么他就会变成候选人并发起一次选举。获得集群中大多数选票的候选人将成为领导人。在一个任期内，领导人一直都会是领导人，直到自己宕机了。

![图 5](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-5.png)
...
在等待投票的时候，候选人可能会从其他的服务器接收到声明它是领导人的附加条目（AppendEntries）RPC。如果这个领导人的任期号（包含在此次的 RPC 中）不小于候选人当前的任期号，那么候选人会承认领导人合法并回到跟随者状态。 如果此次 RPC 中的任期号比自己小，那么候选人就会拒绝这次的 RPC 并且继续保持候选人状态。
...
![图 6](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-6.png)
...
![图 7](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-7.png)
...
如同 5.3 节介绍的那样，当一条日志被复制到大多数的服务器上，领导人就能知道当前任期内的这条日志是可以被提交的。如果领导人在提交日志条目之前 crash 掉了，后续的领导人会继续尝试完成复制这条日志。但是，后续的领导人不能断定之前任期里的一条日志被保存到大多数服务器上后，就一定被马上被提交了。图 8 展示了一种情况，一条已经被存储到大多数节点上的老日志条目，也依然有可能会被未来的领导人覆盖掉。

![图 8](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-8.png)
...
![图 9](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-9.png)
...
![图 10](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-10.png)
...
![图 11](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-11.png)
...
![图 12](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-12.png)
...
![图 13 ](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-13.png)
...
![图 14](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-14.png)
...
![图 15](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-15.png)
...
![图 16](https://baixiangcpp.oss-cn-shanghai.aliyuncs.com/blog/raft-paper/raft-16.png)
